...
function(req, res)
{
  ...
  df = data.frame(
    ...
    GenderMale =  as.integer(data[24]),
    GenderFemale =  as.integer(data[25]),
    GenderJoint =  as.integer(data[26]),
    loan_type1 =  as.integer(data[27]),
    loan_type2 =  as.integer(data[28]),
    loan_type3 =  as.integer(data[29]),
    loan_purpose_p1 =  as.numeric(data[30]),
    loan_purpose_p2 =  as.numeric(data[31]),
    loan_purpose_p3 =  as.numeric(data[32]),
    loan_purpose_p4 =  as.numeric(data[33]),
    occupancy_type_pr =  as.integer(data[34]),
    occupancy_type_ir =  as.integer(data[35]),
    occupancy_type_sr =  as.integer(data[36]),
    credit_type_equi =  as.integer(data[37]),
    credit_type_crif =  as.integer(data[38]),
    credit_type_cib =  as.integer(data[39]),
    credit_type_exp =  as.integer(data[40]),
    age_.25 =  as.numeric(data[41]),
    age_25.34 =  as.numeric(data[42]),
    age_35.44 =  as.numeric(data[43]),
    age_45.54 =  as.numeric(data[44]),
    age_55.64 =  as.numeric(data[45]),
    age_65.74 =  as.numeric(data[46]),
    age_.74 =  as.numeric(data[47]),
    Region_North =  as.integer(data[48]),
    Region_Central =  as.integer(data[49]),
    Region_South =  as.integer(data[50]),
    Region_North.East =  as.integer(data[51])
  )
